...
shearwater_fun = "/lustre/scratch119/humgen/projects/cnv_15x/clonal_hematopoiesis_ibd/newcastle_collaboration/shearwater/scripts/wrapper_shearwaterML_multiplebams_v050220.R"; # Add path to this script if necessary
...
for (h in 1:length(entry_start)) {
    ...
    cmds[length(cmds)+1] = sprintf("bsub -J %s -q normal -R 'select[mem>=5000] rusage[mem=5000]' -M5000 -e bsub.err -o bsub.out %s %s %s %0.0f %0.0f %s %s %s/shearwater_temp_%s", job_id, R_program, shearwater_fun, baits_bed, entry_start[h], entry_end[h], "case.list.tsv", "normal.list.tsv", outdir, analysis_name)
