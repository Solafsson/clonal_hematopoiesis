...
R_program = "/software/R-3.4.0/bin/Rscript"
#
...
shearwater_fun = "../wrapper_shearwaterML_multiplebams_v120719.R"; # Add path to this script if necessary
...
for (h in 1:length(entry_start)) {
	...
	cmds[length(cmds)+1] = sprintf("bsub -J %s -q normal -R 'select[mem>=5000] rusage[mem=5000]' -M5000 -e bsub.err -o bsub.out %s %s %s %0.0f %0.0f %s %s %s/shearwater_temp_%s", job_id, R_program, shearwater_fun, baits_bed, entry_start[h], entry_end[h], "tumor.list.tsv", "normal.list.tsv", ".", analysis_name)
